...
old_df = pd.read_excel('previous scraped data.xlsx')
old_list = list(set(old_df['NAME'].tolist()))
...
df.to_excel('.\raw_data\previous scraped data.xlsx', index =False)
...
df['rent'] = df['rent'].apply(lambda x: x.strip("[]").replace("'","").split(", "))
...
cleaned_df.to_csv('.\clean_data\cleaned results.csv', index=False)
